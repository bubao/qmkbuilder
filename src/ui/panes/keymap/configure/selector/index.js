...
class Selector extends React.Component {
	constructor(props) {
		...
		// Get the category.
		const category = keycode
			? C.KEYCODE_REVERSE_CATEGORIES[keycode.id] || 'OTHER'
			: Object.keys(C.KEYCODE_CATEGORIES)[0]
	...
	setKeycode(id) {
		...
		if (
			C.KEYCODES[id] &&
			(!this.props.limited ||
				(id.startsWith('KC_') && !C.KEYCODE_SPECIAL.includes(id)))
		) {
			// onChange.
	...
	render() {
		...
		for (const category of categoryNames) {
			const className = classNames('pane-keymap-selector-tab', {
		...
		if (this.state.tab === 'OTHER') {
			...
			for (
				let i = 0;
				i < C.KEYCODE_CATEGORIES[this.state.tab].length;
				i++
			) {
				const key = C.KEYCODE_CATEGORIES[this.state.tab][i]
				if (key) {
					if (this.props.limited && C.KEYCODE_SPECIAL.includes(key))
						continue // In limited mode, no special keycodes.
					const buttonContent =
						this.state.tab === 'FN' ? key : C.KEYCODES[key].display
					const style = {}
					if (keycode && key === keycode.id)
		...
		return (
			<div className="pane-keymap-selector" style={this.props.style}>
				<div
				...
				{tabs}
				<div className="pane-keymap-selector-buttons">{content}</div>
			</div>
		)
	}
}
