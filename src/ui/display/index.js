...
class Display extends React.Component {
	...
	zoom(direction) {
		...
		return () => {
			...
			// Bound the size.
			keySize = Math.min(
				Math.max(keySize, C.KEY_SIZE_MIN),
				C.KEY_SIZE_MAX
			)
					...
					const keyListener = e => {
						if (key.selected === C.KEY_PROGRAM) {
							...
							if (C.KEYCODE_NUMBERS[e.which]) {
								...
								key.keycodes[layer] = Keycode.getDefault(
									C.KEYCODE_NUMBERS[e.which]
								)
								keyboard.verify()
							}
							key.select(C.KEY_SELECT)
			...
			<div className="display-wrapper">
				...
				<label
					onClick={() => {
						state.ui.set(
							'display-flip',
							!state.ui.get('display-flip', false)
						)
					}}
				/>
				<br />
